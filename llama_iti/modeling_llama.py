...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if use_grad:
                inputs_embeds = Variable(inputs_embeds, requires_grad=True)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if use_grad:
                ...
                targets = torch.ones((shift_logits.shape[0], self.config.vocab_size)).cuda()[shift_labels !=-100]
                shift_logits = shift_logits[shift_labels !=-100] / 1
                loss = torch.mean(torch.mean(-targets * logsoftmax(shift_logits), dim=-1))
                ####gradnorm loss########
            else:
                # Flatten the tokens
